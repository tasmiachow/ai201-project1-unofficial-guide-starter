...
def retrieve(query, n_results=N_RESULTS):
    ...
    Return a list of dicts, each with:
      - "text"     : the chunk text
      - "park       : the park name(s) from metadata
    ...
    res_query = _collection.query(query_texts=query, n_results=n_results, include=["documents", "metadatas", "distances"])
    res_docs = res_query["documents"][0]
    res_meta = res_query["metadatas"][0]
    res_distances = res_query["distances"][0]
    
    
    format_dict = []

    for doc, meta, distance in zip(res_docs, res_meta, res_distances): 
        local_dic = {
            "text" : doc, 
            "park" : meta["park"], 
            "source" : meta["source"],
...
print(retrieve(query="How many quiet zones are there in central park?", n_results=2))